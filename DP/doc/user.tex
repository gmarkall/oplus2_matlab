...
\begin{routine} {op\_init(int argc, char **argv, int diags\_level)}
...
\begin{routine} {op\_decl\_map(op\_set from, op\_set to, int dim, int *imap, op\_map map,\\
\hspace*{1in} char *name)}
...
\begin{routine} {op\_decl\_dat(op\_set set, int dim, char *type, T *idat, op\_dat dat,\\
\hspace*{0.9in} char *name)}
