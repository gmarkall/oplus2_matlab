...
%Surprisingly, I think this is perhaps the optimal implementation.
%If the values being reduced are random variables, the probability that the
%$n^{th}$ thread needs to update the global minimum (or maximum) is $1/n$.
%Hence, if there are $N$ threads in total the average number of global updates 
%would be
%\[
%\sum_{n=1}^N 1/n\ \approx\ \log n.
%\]
%Doing a local reduction within each thread block would add an $O(N)$ 
%cost which would be larger in most cases than the $O(\log N)$ cost
%of the atomic updates.
%
%\subsection{Alternative implementation}
%
%Instead of using atomic operations an alternative would be for each 
%thread block to compute its sum/min/max and put it into a global array
%for the CPU to transfer back and complete the reduction. I should probably 
%investigate this approach sometime.  One advantage of this is that it 
%would be much simpler for the user-defined datatypes discussed in the next section.
...
% provided the user provides an 
%appropriate version of {\tt atomicAdd} for the user-defined datatype.
%For datatypes which are longer than 8 bytes, I think this will require
%the use of some kind of mutex lock.  
%
%One possibility is to add a boolean lock as an extra component within 
%the datatype. The lock can be set by a busy-wait loop using {\tt atomicCAS}.
%The rest of the datatype can then be updated safely before releasing 
%the lock.  I think {\tt \_\_threadfence()} may be needed before the lock 
%release to ensure the previous write operations have completed; see 
%section B.5 in the CUDA Programming Guide version 3.0.
%
%A slight alternative is to use an existing element of the datatype as 
%a lock.  For example, a NaN value for a floating point element could 
%be used to indicate a lock.
