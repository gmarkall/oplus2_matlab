...
__global__ void op_cuda_res_calc(                                                 
  ...
  */
  ...
  int n_max = MAX(MAX(ind_arg0_size*2,ind_arg1_size*4),
                  MAX(ind_arg2_size*1,ind_arg3_size*4));

  for (int n=threadIdx.x; n<n_max; n+=blockDim.x) {
    float f0, f1, f2;
    bool  b0 = n<ind_arg0_size*2;
    bool  b1 = n<ind_arg1_size*4;
    bool  b2 = n<ind_arg2_size*1;
    bool  b3 = n<ind_arg3_size*4;

    if (b0) f0 = ind_arg0[n%2+ind_arg0_map[n/2]*2];                         
    if (b1) f1 = ind_arg1[n%4+ind_arg1_map[n/4]*4];
    if (b2) f2 = ind_arg2[n%1+ind_arg2_map[n/1]*1];

    if (b0) ind_arg0_s[n] = f0;
    if (b1) ind_arg1_s[n] = f1;
    if (b2) ind_arg2_s[n] = f2;
    if (b3) ind_arg3_s[n] = ZERO_float;
  }
...
void op_par_loop_res_calc(char const *name, op_set set,                           
  ...
  for (int col=0; col<(*Plan).ncolors; col++) {                                   
    ...
    op_cuda_res_calc<<<nblocks,nthread,nshared>>>(                                
       ...
       (float *)arg4.dat_d, (*Plan).ind_maps[2],                                  
       (float *)arg6.dat_d, (*Plan).ind_maps[3],                                  
       (*Plan).maps[0],                                                           
       (*Plan).maps[1],                                                           
       (*Plan).maps[2],                                                           
       (*Plan).maps[3],                                                           
