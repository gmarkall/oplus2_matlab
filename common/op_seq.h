//
// auto-generated by op_seq_gen.m on 24-Mar-2011 23:27:24
//
...
//
// Auxiliary structs to strip extents form kernel parameters
//
...
template < typename D >
void allocate_mem(const op_map& map, const D* dat, char** p) {}

void allocate_mem(const op_map& map, const op_dat& arg, char** p) {
  *p = (char*) malloc(map.dim * sizeof(char*));
}
...
inline void copy_in(int n,op_dat arg,op_map map, char *p_arg){
  if (arg.dim == 1) {
    ...
    // For each index in the target dimension of map, get pointer to the data
    char** p = (char**) p_arg;
    for (int i = 0; i < map.dim; ++i) {
      p[i] = arg.dat + map.map[i+n*map.dim]*arg.size;
    }
  }
}

template < typename T >
inline void copy_in(int n,T* arg,op_map map, char *p_arg){}
...
void op_par_loop (void (*kernel)( T0 ),
  ...
  if (OP_diags>0) {
    arg_check(set,0 ,name,arg0 ,idx0 ,map0 ,dim0 ,typ0 ,acc0 , (typename RemoveExtent<T0 >::type)p_arg0 ,&ninds);
  }
...
void op_par_loop (void (*kernel)( T0, T1 ),
  ...
  if (OP_diags>0) {
    arg_check(set,0 ,name,arg0 ,idx0 ,map0 ,dim0 ,typ0 ,acc0 , (typename RemoveExtent<T0 >::type)p_arg0 ,&ninds);
    arg_check(set,1 ,name,arg1 ,idx1 ,map1 ,dim1 ,typ1 ,acc1 , (typename RemoveExtent<T1 >::type)p_arg1 ,&ninds);
  }
...
void op_par_loop (void (*kernel)( T0, T1, T2 ),
  ...
  if (OP_diags>0) {
    arg_check(set,0 ,name,arg0 ,idx0 ,map0 ,dim0 ,typ0 ,acc0 , (typename RemoveExtent<T0 >::type)p_arg0 ,&ninds);
    arg_check(set,1 ,name,arg1 ,idx1 ,map1 ,dim1 ,typ1 ,acc1 , (typename RemoveExtent<T1 >::type)p_arg1 ,&ninds);
    arg_check(set,2 ,name,arg2 ,idx2 ,map2 ,dim2 ,typ2 ,acc2 , (typename RemoveExtent<T2 >::type)p_arg2 ,&ninds);
  }
...
void op_par_loop (void (*kernel)( T0, T1, T2, T3 ),
  ...
  if (OP_diags>0) {
    ...
    arg_check(set,1 ,name,arg1 ,idx1 ,map1 ,dim1 ,typ1 ,acc1 , (typename RemoveExtent<T1 >::type)p_arg1 ,&ninds);
    arg_check(set,2 ,name,arg2 ,idx2 ,map2 ,dim2 ,typ2 ,acc2 , (typename RemoveExtent<T2 >::type)p_arg2 ,&ninds);
    arg_check(set,3 ,name,arg3 ,idx3 ,map3 ,dim3 ,typ3 ,acc3 , (typename RemoveExtent<T3 >::type)p_arg3 ,&ninds);
  }
...
void op_par_loop (void (*kernel)( T0, T1, T2, T3, T4 ),
  ...
  if (OP_diags>0) {
    ...
    arg_check(set,2 ,name,arg2 ,idx2 ,map2 ,dim2 ,typ2 ,acc2 , (typename RemoveExtent<T2 >::type)p_arg2 ,&ninds);
    arg_check(set,3 ,name,arg3 ,idx3 ,map3 ,dim3 ,typ3 ,acc3 , (typename RemoveExtent<T3 >::type)p_arg3 ,&ninds);
    arg_check(set,4 ,name,arg4 ,idx4 ,map4 ,dim4 ,typ4 ,acc4 , (typename RemoveExtent<T4 >::type)p_arg4 ,&ninds);
  }
...
void op_par_loop (void (*kernel)( T0, T1, T2, T3, T4,
  ...
  if (OP_diags>0) {
    ...
    arg_check(set,3 ,name,arg3 ,idx3 ,map3 ,dim3 ,typ3 ,acc3 , (typename RemoveExtent<T3 >::type)p_arg3 ,&ninds);
    arg_check(set,4 ,name,arg4 ,idx4 ,map4 ,dim4 ,typ4 ,acc4 , (typename RemoveExtent<T4 >::type)p_arg4 ,&ninds);
    arg_check(set,5 ,name,arg5 ,idx5 ,map5 ,dim5 ,typ5 ,acc5 , (typename RemoveExtent<T5 >::type)p_arg5 ,&ninds);
  }
