

function op_seq_gen()

%
% this sets the max number of arguments in op_par_loop
%

maxargs = 6;

%
% first the top bit with headers and templates
%

file = strvcat(...',...
'//',...
'// headers',...
'//',...
' ',...
'#include <stdlib.h>',...
'#include <stdio.h>',...
'#include <string.h>',...
'#include <math.h>',...
'#include "op_datatypes.h"',...
' ',...
'//',...
'// Auxiliary structs to strip extents form kernel parameters',...
'//',...
' ',...
'template<typename T>',...
'struct RemoveExtent',...
'{',...
'  typedef T type;',...
'};',...
' ',...
'template<typename T>',...
'struct RemoveExtent<T**>',...
'{',...
'  typedef typename RemoveExtent<T*>::type type;',...
'};',...
' ',...
'template<typename T, size_t N>',...
'struct RemoveExtent<T[N]>',...
'{',...
'  typedef typename RemoveExtent<T>::type type;',...
'};',...
' ',...
'template<typename T, size_t N>',...
'struct RemoveExtent<T(*)[N]>',...
'{',...
  'typedef typename RemoveExtent<T*>::type type;',...
'};',...
' ',...
'//',...
'// templates',...
'//',' ',...
'template < class T, class D >',...
'void arg_check(op_set set, int m, const char *name, D *arg, int idx, op_map map,',...
'               int dim, const char *typ, op_access acc, T p_arg, int *ninds){',...
'  if (idx != OP_NONE) {',...
'    printf("error: arg %d in kernel \"%s\"\n",m,name);',...
'    printf("invalid index, should be OP_NONE for constant arg\n");',...
'    exit(1);',...
'  }',...
'  if (type_error(arg,typ)) {',...
'    printf("error: arg %d in kernel \"%s\"\n",m,name);',...
'    printf("data type does not match type of input dataset\n");',...
'    exit(1);',...
'  }',...
'  if (type_error(p_arg,typ)) {',...
'    printf("error: arg %d in kernel \"%s\"\n",m,name);',...
'    printf("data type does not match type of function argument\n");',...
'    exit(1);',...
'  }',...
'  if (dim <= 0) {',...
'    printf("error: arg %d in kernel \"%s\"\n",m,name);',...
'    printf("dimension must be strictly positive\n");',...
'    exit(1);',...
'  }',...
'}',...
' ',...
'template < class T >',...
'void arg_check(op_set set, int m, const char *name, op_dat arg, int idx, op_map map,',...
'               int dim, const char *typ, op_access acc, T p_arg, int *ninds){',...
'  if (idx>=0 || idx==OP_ALL) (*ninds)++;',...
' ',...
'  if (map.map == NULL) {',...
'    if (idx != OP_NONE) {',...
'      printf("error: arg %d in kernel \"%s\"\n",m,name);',...
'      printf("invalid index, should be OP_NONE for identity mapping\n");',...
'      exit(1);',...
'    }',...
'  }',...
'  else {',...
'    if (set.index != map.from.index || arg.set.index != map.to.index) {',...
'      printf("error: arg %d in kernel \"%s\"\n",m,name);',...
'      printf("invalid pointer\n");',...
'      exit(1);',...
'    }',...
'    if (idx != OP_ALL && (idx < 0 || idx >= map.dim)) {',...
'      printf("error: arg %d in kernel \"%s\"\n",m,name);',...
'      printf("invalid pointer index\n");',...
'      exit(1);',...
'    }',...
'  }',...
'  if (strcmp(arg.type,typ)) {',...
'    printf("error: arg %d in kernel \"%s\"\n",m,name);',...
'    printf("data type does not match type of input dataset\n");',...
'    exit(1);',...
'  }',...
'  if (type_error(p_arg,typ)) {',...
'    printf("error: arg %d in kernel \"%s\"\n",m,name);',...
'    printf("data type does not match type of function argument\n");',...
'    exit(1);',...
'  }',...
'  if (arg.dim != dim) {',...
'    printf("error: arg %d in kernel \"%s\"\n",m,name);',...
'    printf("dimension does not match input dataset\n");',...
'    exit(1);',...
'  }',...
'}',...
' ',...
'template < typename D >',...
'void allocate_mem(const op_map& map, const D* dat, char** p) {}',...
' ',...
'void allocate_mem(const op_map& map, const op_dat& arg, char** p) {',...
'  *p = (char*) malloc(map.dim * sizeof(char*));',...
'}',...
' ',...
'template < class D >',...
'inline void arg_set(int n,D *arg,int idx,op_map map,int dim,',...
'             const char *typ,op_access acc,char **p_arg){',...
'  *p_arg = (char *) arg;',...
'}',...
' ',...
'inline void arg_set(int n,op_dat arg,int idx,op_map map,int dim,',...
'             const char *typ,op_access acc,char **p_arg){',...
'  int n2;',...
'  if (map.dim==0)                   // identity mapping',...
'    n2 = n;',...
'  else                              // standard pointers',...
'    n2 = map.map[idx+n*map.dim];',...
' ',...
'  *p_arg = arg.dat + n2*arg.size;',...
'}',...
' ',...
'inline void copy_in(int n,op_dat arg,op_map map, char *p_arg){',...
'  if (arg.dim == 1) {',...
'    for (int i = 0; i < map.dim; ++i)',...
'      memcpy(p_arg + i*arg.size, arg.dat + map.map[i+n*map.dim]*arg.size, arg.size);',...
'  } else {',...
'    // For each index in the target dimension of map, get pointer to the data',...
'    char** p = (char**) p_arg;',...
'    for (int i = 0; i < map.dim; ++i) {',...
'      p[i] = arg.dat + map.map[i+n*map.dim]*arg.size;',...
'    }',...
'  }',...
'}',...
' ',...
'template < typename T >',...
'inline void copy_in(int n,T* arg,op_map map, char *p_arg){}',...
' ');

%
% now for op_par_loop defns
%

for nargs = 1:maxargs

  file = strvcat(file,' ', ...
    '// ',...
   ['// op_par_loop routine for ' num2str(nargs) ' arguments '],...
    '// ',' ');

  n_per_line = 5;

  line = 'template < ';
  for n = 1:nargs
    line = [ line 'class T' num2str(n-1) ','];
    if (mod(n,n_per_line)==0 || n==nargs)
      file = strvcat(file,line);
      line = '           ';
    elseif (n<=10)
      line = [line ' '];
    end
  end
  for n = 1:nargs
    line = [ line 'class D' num2str(n-1) ','];
    if (n==nargs)
      line = [line(1:end-1) ' >'];
    end
    if (mod(n,n_per_line)==0 || n==nargs)
      file = strvcat(file,line);
      line = '           ';
    elseif (n<=10)
      line = [line ' '];
    end
  end

  line = ['void op_par_loop (void (*kernel)( '];
  for n = 1:nargs
    line = [ line 'T' num2str(n-1) ','];
    if (n==nargs)
      line = [line(1:end-1) ' ),'];
    end
    if (mod(n,n_per_line)==0 || n==nargs)
      file = strvcat(file,line);
      line = '                                    ';
    elseif (n<=10)
      line = [line ' '];
    end
  end

  file = strvcat(file,'  char const * name, op_set set,');

  for n = 1:nargs
    line = '  D0 arg0,int idx0,op_map map0,int dim0,const char *typ0,op_access acc0,';
    if (n==nargs)
      line = [line(1:end-1) '){'];
    end
    num  = sprintf('%d ',n-1);
    file = strvcat(file,regexprep(line,'0',num(1:2)));
  end

  file = strvcat(file,' ');

  line = '  char ';
  for n = 1:nargs
    line = [ line '*p_arg' num2str(n-1) ',' ];
    if (n==nargs)
      line = [line(1:end-1) ';'];
    end
    if (mod(n,n_per_line)==0 || n==nargs)
      file = strvcat(file,line);
      line = '       ';
    elseif (n<=10)
      line = [line ' '];
    end
  end

%
% diagnostics
%

  file = strvcat(file,' ',...
                      '  int ninds=0;',' ',...
                      '  // consistency checks',' ',...
                      '  if (OP_diags>0) {');

  for n = 1:nargs
    line = '    arg_check(set,0,name,arg0,idx0,map0,dim0,typ0,acc0, (typename RemoveExtent<T0>::type)p_arg0,&ninds);';
    num  = sprintf('%d ',n-1);
    file = strvcat(file,regexprep(line,'0',num(1:2)));
  end

  file = strvcat(file,'  }',' ',...
                      '  if (OP_diags>2) {',...
                      '    if (ninds==0)',...
                      '      printf(" kernel routine w/o indirection:  %s \n",name);',...
                      '    else',...
                      '      printf(" kernel routine with indirection: %s \n",name);',...
                      '  }',' ');

%
% Allocate space for copy-in
%

  file = strvcat(file,'  // Allocate space for copy-in');
  for n = 1:nargs
    line = '  if (idx0 == OP_ALL) allocate_mem(map0, arg0, &p_arg0);';
    num  = sprintf('%d ',n-1);
    file = strvcat(file,regexprep(line,'0',num(1:2)));
  end

%
% main loop
%

  file = strvcat(file,' ', '  // loop over set elements',' ',...
                      '  for (int n=0; n<set.size; n++) {');
  for n = 1:nargs
    num  = sprintf('%d ',n-1);
    line = '    if (idx0 == OP_ALL)';
    file = strvcat(file,regexprep(line,'0',num(1:2)));
    line = '      copy_in(n, arg0, map0, p_arg0);';
    file = strvcat(file,regexprep(line,'0',num(1:2)));
    file = strvcat(file,'    else');
    line = '      arg_set(n,arg0,idx0,map0,dim0,typ0,acc0,&p_arg0);';
    file = strvcat(file,regexprep(line,'0',num(1:2)));
  end

%
% call to user's kernel
%

  n_per_line = 5;

  file = strvcat(file,' ','    // call kernel function, passing in pointers to data',' ');

  line = ['    kernel( '];
  for n = 1:nargs
    line = [ line 'T' num2str(n-1) '(p_arg'  num2str(n-1) '),'];
    if (n==nargs)
      line = [line(1:end-1) ' );'];
    end
    if (mod(n,n_per_line)==0 || n==nargs)
      file = strvcat(file,line);
      line = '            ';
    elseif (n<=10)
      line = [line '  '];
    end
  end

  file = strvcat(file,'  }',' ');

%
% Free space for copy-in
%

  file = strvcat(file,'  // Free space for copy-in');
  for n = 1:nargs
    line = '  if (idx0 == OP_ALL) free(p_arg0);';
    num  = sprintf('%d ',n-1);
    file = strvcat(file,regexprep(line,'0',num(1:2)));
  end

%
% Lead out
%

  file = strvcat(file,'}');

end


%
% print out into file
%


fid = fopen('op_seq.h','wt');
fprintf(fid,'// \n// auto-generated by op_seq_gen.m on %s \n//\n\n',datestr(now));
for n=1:size(file,1)
  fprintf(fid,'%s\n',file(n,:));
end
fclose(fid);

