...
__global__ void op_cuda_bres_calc(                                                         
  ...
  for (int n=threadIdx.x; n<nelems2; n+=blockDim.x) {                                      
    ...
    if (n<nelem) {                                                                         
      ...
      bres_calc( ind_arg0_s+arg0_ptr[n]*2,                                                 
                 ind_arg0_s+arg1_ptr[n]*2,                                                 
                 ind_arg1_s+arg2_ptr[n]*4,                                                 
                 ind_arg2_s+arg3_ptr[n]*1,                                                 
                 arg4_l,                                                                   
                 arg5+n*1 );                                                               
    ...
    for (int col=0; col<ncolor; col++) {                                                   
      if (col2==col) {                                                                     
        for (int d=0; d<4; d++)                                                            
          ind_arg3_s[d+arg4_ptr[n]*4] += arg4_l[d];                                        
      }                                                                                    
      __syncthreads();                                                                     
    }                                                                                      
...
void op_par_loop_bres_calc(char const *name, op_set set,                                   
  ...
  for (int col=0; col<(*Plan).ncolors; col++) {                                            
    ...
    op_cuda_bres_calc<<<nblocks,nthread,nshared>>>(                                        
       ...
       (float *)arg3.dat_d, (*Plan).ind_ptrs[2], (*Plan).ind_sizes[2], (*Plan).ind_offs[2],
       (float *)arg4.dat_d, (*Plan).ind_ptrs[3], (*Plan).ind_sizes[3], (*Plan).ind_offs[3],
       (*Plan).ptrs[0],                                                                    
       (*Plan).ptrs[1],                                                                    
       (*Plan).ptrs[2],                                                                    
       (*Plan).ptrs[3],                                                                    
